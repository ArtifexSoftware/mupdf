...
C++ wrapping:
    ...
    Details:
        ...
        Internal `fz_context*`'s:
            ...
            `platforms/c++/implementation/internal.cpp:internal_context_get()`.
...
Windows builds:
    ...
    DLL export of functions and data:
        ...
        platforms/c++/windows_mupdf.def file; this lists all C public global
        data.
    ...
    Building the DLLs:
        ...
        by platforms/win32/mupdfpyswig.vcxproj. Thus one cannot easily build
...
Usage:
    ...
    Args:
        ...
        --build [<args>] <actions>:
            ...
            order. If <actions> is 'all', it is replaced by m0123.
                ...
                0:
                    ...
                    mupdf/platforms/c++/{implementation,include}.
                ...
                2:
                    ...
                    For example for Python this generates:
                        ...
                        mupdf/platforms/python/mupdfcpp_swig.i
                        mupdf/platforms/python/mupdfcpp_swig.i.cpp
                ...
                3:
                    ...
                    Along with mupdf/platforms/python/mupdf.py (generated by
        ...
        --doc <languages>
            ...
            <languages> is either 'all' or a comma-separated list of API languages:
                ...
                c++
                    Generate documentation for the C++ API with doxygen:
                        platforms/c++/include/html/index.html
                python
                    Generate documentation for the Python API using pydoc3:
                        platforms/python/mupdf.html
        ...
        --vs-upgrade 0 | 1
            ...
            `platforms/win32/` called `platforms/win32-vs-upgrade`, modifying the
    ...
    Examples:
        ...
        ./scripts/mupdfwrap.py -b 0 --compare-fz_usage platforms/gl
            Compare generated class methods with functions called by platforms/gl
            code.
...
def _windows_vs_upgrade( vs_upgrade, build_dirs, devenv):
    ...
    {build_dirs.dir_mupdf}/platforms/win32-vs-upgrade/ tree with upgraded .sln
    ...
    if infix is None:
        ...
        prefix1 = f'{build_dirs.dir_mupdf}/platforms/win32/'
        prefix2 = f'{build_dirs.dir_mupdf}/platforms/{infix}/'
...
def build_0(
    ...
    cpp.cpp_source(
            ...
            namespace,
            f'{build_dirs.dir_mupdf}/platforms/c++',
            header_git,
            generated,
            check_regress,
            clang_info_verbose,
    ...
    generated.save(f'{build_dirs.dir_mupdf}/platforms/c++')
    ...
    for dir_ in (
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/', '.h',
            ):
...
def build( build_dirs, swig_command, args, vs_upgrade, make_command):
    ...
    '''
    cpp_files   = [
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/classes.cpp',
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/classes2.cpp',
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/exceptions.cpp',
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/functions.cpp',
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/internal.cpp',
            f'{build_dirs.dir_mupdf}/platforms/c++/implementation/extra.cpp',
            ]
    h_files = [
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/classes.h',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/classes2.h',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/exceptions.h',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/functions.h',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/internal.h',
            f'{build_dirs.dir_mupdf}/platforms/c++/include/mupdf/extra.h',
            ]
    build_python = True
    build_csharp = False
    check_regress = False
            ...
            if action == '.':
                ...
                if state.state_.windows:
                    ...
                    command = (
                            ...
                            f'"{devenv}"'
                            f' platforms/{win32_infix}/mupdf.sln'
                            f' /Build "{build}"'
                            )
                    ...
                    jlib.fs_copy(
                            f'{build_dirs.dir_mupdf}/platforms/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/mupdfcpp{build_dirs.cpu.windows_suffix}.dll',
                            f'{build_dirs.dir_so}/',
                            verbose=1,
                            )
                ...
                #generated = cpp.Generated(f'{build_dirs.dir_mupdf}/platforms/c++')
                with open( f'{build_dirs.dir_mupdf}/platforms/c++/generated.pickle', 'rb') as f:
                ...
                if state.state_.windows:
                    if build_python:
                        ...
                        if 1:
                            ...
                            path_o = f'{path_cpp}.o'
                            mupdfcpp_lib = f'{build_dirs.dir_mupdf}/platforms/win32/'
                            ...
                            build_so_windows(
                                    ...
                                    includes = (
                                        ...
                                        f'{build_dirs.dir_mupdf}/platforms/c++/include',
                                        wp.include,
                                    ),
                                    libs = mupdfcpp_lib,
                                    libpaths = wp.libs,
                            ...
                            command = (
                                    f'cd {build_dirs.dir_mupdf}&&'
                                    f'"{devenv}"'
                                    f' platforms/{win32_infix}/mupdfpyswig.sln'
                            ...
                            jlib.fs_copy(
                                    f'{build_dirs.dir_mupdf}/platforms/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/mupdfpyswig.dll',
                    ...
                    if build_csharp:
                        ...
                        if 1:
                            ...
                            command = (
                                    f'cd {build_dirs.dir_mupdf}&&'
                                    f'"{devenv}"'
                                    f' platforms/{win32_infix}/mupdfcsharpswig.sln'
                            ...
                            jlib.fs_copy(
                                    f'{build_dirs.dir_mupdf}/platforms/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/mupdfcsharpswig.dll',
                    ...
                    # mupdf:platforms/python/_mupdf.so. We could instead
                    ...
                    # Build mupdf2.so
                    if build_python:
                        cpp2_path = f'{build_dirs.dir_mupdf}/platforms/python/mupdfcpp2_swig.cpp'
...
def make_docs( build_dirs, languages_original):
    ...
    for language in languages:
        ...
        if language == 'c':
            ...
            do_doxygen( 'mupdfcpp', f'{out_dir}/c++', f'{build_dirs.dir_mupdf}/platforms/c++/include')
        ...
        with jlib.LogPrefixScope( f'{arg}: '):
            ...
            if arg == '-h' or arg == '--help':
                ...
                for path in jlib.fs_paths( build_dirs.ref_dir):
                    ...
                    path2 = f'{build_dirs.dir_mupdf}/platforms/c++/{tail}'
                ...
                for a, b in (
                        (f'{build_dirs.dir_mupdf}/platforms/c++/', f'{build_dirs.dir_mupdf}/platforms/c++/'),
                        (f'{build_dirs.dir_mupdf}/platforms/python/', f'{build_dirs.dir_mupdf}/platforms/python/')
                        ):
                ...
                jlib.system(
                        f'rsync -ai {build_dirs.dir_mupdf}/platforms/c++/implementation {build_dirs.ref_dir}',
                        out='log',
                        )
                jlib.system(
                        f'rsync -ai {build_dirs.dir_mupdf}/platforms/c++/include {build_dirs.ref_dir}',
                        out='log',
                        )
                ...
                files = generated.h_files + generated.cpp_files + [
                        ...
                        f'{build_dirs.dir_mupdf}/platforms/c++/fn_usage.txt',
                        ]
                ...
                includes = (
                        ...
                        f' -I {build_dirs.dir_mupdf}/platforms/c++/include'
                        )
                cpp_flags = build_dirs.cpp_flags
                if state.state_.windows:
                    win32_infix = _windows_vs_upgrade( vs_upgrade, build_dirs, devenv=None)
                    windows_build_type = build_dirs.windows_build_type()
                    lib = f'{build_dirs.dir_mupdf}/platforms/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/mupdfcpp{build_dirs.cpu.windows_suffix}.lib'
                    vs = wdev.WindowsVS()
                    command = textwrap.dedent(f'''
                            "{vs.vcvars}"&&"{vs.cl}"
                                /Tp{src}
                ...
                if arg == '--test-python-gui':
                    ...
                    with open( f'{build_dirs.dir_mupdf}/platforms/python/mupdf_test.py.out.txt', 'w') as f:
                        jlib.system( command, env_extra=env_extra, out='log', verbose=1)
