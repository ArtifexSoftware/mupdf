...
static void
pdf_dev_color(fz_context *ctx, pdf_device *pdev, fz_colorspace *colorspace, const float *color, int stroke, const fz_color_params *cs_params)
{
	int diff = 0;
	int i;
	int cspace = 0;
	...
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	pdf_dev_color(ctx, pdev, colorspace, color, 0, cs_params);
	...
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	pdf_dev_color(ctx, pdev, colorspace, color, 1, cs_params);
	pdf_dev_ctm(ctx, pdev, ctm);
...
static void
pdf_dev_fill_image(fz_context *ctx, fz_device *dev, fz_image *image, const fz_matrix *ctm, float alpha, const fz_color_params *cs_params)
{
	pdf_device *pdev = (pdf_device*)dev;
	pdf_obj *im_res;
	gstate *gs = CURRENT_GSTATE(pdev);
...
static void
pdf_dev_fill_shade(fz_context *ctx, fz_device *dev, fz_shade *shade, const fz_matrix *ctm, float alpha, const fz_color_params *cs_params)
{
	pdf_device *pdev = (pdf_device*)dev;
...
static void
pdf_dev_begin_mask(fz_context *ctx, fz_device *dev, const fz_rect *bbox, int luminosity, fz_colorspace *colorspace, const float *color, const fz_color_params *cs_params)
{
	pdf_device *pdev = (pdf_device*)dev;
	pdf_document *doc = pdev->doc;
	gstate *gs;
