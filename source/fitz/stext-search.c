...
fz_search_result fz_search_forwards(fz_context *ctx, fz_search *search)
{
	...
	if (spun_begin)
	{
		...
		while (lookup_search_ix(search, begin_ix) == NULL && begin_ix < end_ix) ++begin_ix;
		assert(lookup_search_ix(search, begin_ix) != NULL);
		while (lookup_search_ix(search, end_ix) == NULL && end_ix > begin_ix) --end_ix;
...
fz_search_result fz_search_backwards(fz_context *ctx, fz_search *search)
{
	...
	if (spun_end)
	{
		...
		/* Skip over positions that have no matching stext character (inserted line ends, etc) */
		while (lookup_backward_search_ix(search, begin_ix) == NULL && begin_ix < end_ix) ++begin_ix;
		assert(lookup_backward_search_ix(search, begin_ix) != NULL);
		while (lookup_backward_search_ix(search, end_ix) == NULL && end_ix > begin_ix) --end_ix;
