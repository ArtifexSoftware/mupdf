...
fz_stext_block_iterator fz_stext_block_iterator_next_dfs(fz_stext_block_iterator pos)
{
	while (1)
	{
		pos = fz_stext_block_iterator_next(pos);
...
*/
static fz_stext_block_iterator
fz_stext_block_iterator_next_or_up(fz_stext_block_iterator pos)
{
	while (1)
	{
		pos = fz_stext_block_iterator_next(pos);
		...
		{
			pos = fz_stext_block_iterator_up(pos);
			continue;
		}
		break;
	}
...
fz_stext_block *
insert_new_struct(fz_context *ctx, fz_stext_block_iterator pos, fz_structure classification)
{
...
relink_block_and_next(fz_stext_block_iterator pos, fz_stext_block *newblock)
{
...
static int
split_text_by_rect(fz_context *ctx, fz_stext_block_iterator pos, fz_rect rect)
{
	fz_stext_block *b = pos.pos;
	fz_stext_line *line;
	fz_stext_char *ch;
...
{
	fz_stext_block_iterator pos = fz_stext_block_iterator_begin(page);
	fz_stext_block *newblock;
	...
	while (pos.pos != NULL)
	{
		...
		while (pos.pos != NULL)
		{
			...
			else if (fz_overlaps_rect(pos.pos->bbox, rect))
			{
				...
				{
					/* It's a struct node. */
					pos = fz_stext_block_iterator_down(pos);
			...
			pos = fz_stext_block_iterator_next_or_up(pos);
		}
