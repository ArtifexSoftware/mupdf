...
static void
fz_test_color(fz_context *ctx, fz_test_device *t, fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color)
{
	...
	fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color, float alpha)
{
	...
	if (dev->resolved == 0 && alpha != 0.0f)
	{
		...
		fz_test_color(ctx, dev, colorspace, cs_params, color);
	}
	if (dev->passthrough)
		fz_fill_path(ctx, dev->passthrough, path, even_odd, ctm, colorspace, cs_params, color, alpha);
}
	...
	const fz_matrix *ctm, fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color, float alpha)
{
	...
	if (dev->passthrough)
		fz_stroke_path(ctx, dev->passthrough, path, stroke, ctm, colorspace, cs_params, color, alpha);
}
	...
	fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color, float alpha)
{
	...
	if (dev->passthrough)
		fz_fill_text(ctx, dev->passthrough, text, ctm, colorspace, cs_params, color, alpha);
}
	...
	const fz_matrix *ctm, fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color, float alpha)
{
	...
	if (dev->passthrough)
		fz_stroke_text(ctx, dev->passthrough, text, stroke, ctm, colorspace, cs_params, color, alpha);
}
...
prepare_vertex(fz_context *ctx, void *arg_, fz_vertex *v, const float *color)
{
	...
	if (!shade->use_function)
		fz_test_color(ctx, dev, shade->colorspace, arg->cs_params, color);
}

static void
fz_test_fill_shade(fz_context *ctx, fz_device *dev_, fz_shade *shade, const fz_matrix *ctm, const fz_color_params *cs_params, float alpha)
{
	fz_test_device *dev = (fz_test_device*)dev_;
	...
	if (dev->resolved == 0)
	{
		...
		else
		{
			if (shade->use_function)
			{
				int i;
				for (i = 0; i < 256; i++)
					fz_test_color(ctx, dev, shade->colorspace, cs_params, shade->function[i]);
			}
			else
			{
				struct shadearg arg;
...
static void
fz_test_fill_image(fz_context *ctx, fz_device *dev_, fz_image *image, const fz_matrix *ctm, const fz_color_params *cs_params, float alpha)
{
	fz_test_device *dev = (fz_test_device*)dev_;
	...
	fz_colorspace *colorspace, const fz_color_params *cs_params, const float *color, float alpha)
{
	...
	if (dev->passthrough)
		fz_fill_image_mask(ctx, dev->passthrough, image, ctm, colorspace, cs_params, color, alpha);
}
...
static void
fz_test_begin_mask(fz_context *ctx, fz_device *dev_, const fz_rect *rect, int luminosity, fz_colorspace *cs, const fz_color_params *cs_params, const float *bc)
{
	fz_test_device *dev = (fz_test_device*)dev_;

	fz_begin_mask(ctx, dev->passthrough, rect, luminosity, cs, cs_params, bc);
}
