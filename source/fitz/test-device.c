...
static void
fz_test_color(fz_context *ctx, fz_test_device *t, fz_colorspace *colorspace, const float *color, const fz_color_params *cs_params)
{
	...
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	if (dev->resolved == 0 && alpha != 0.0f)
	{
		...
		fz_test_color(ctx, dev, colorspace, color, cs_params);
	}
	if (dev->passthrough)
		fz_fill_path(ctx, dev->passthrough, path, even_odd, ctm, colorspace, color, alpha, cs_params);
}
	...
	const fz_matrix *ctm, fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	if (dev->passthrough)
		fz_stroke_path(ctx, dev->passthrough, path, stroke, ctm, colorspace, color, alpha, cs_params);
}
	...
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	if (dev->passthrough)
		fz_fill_text(ctx, dev->passthrough, text, ctm, colorspace, color, alpha, cs_params);
}
	...
	const fz_matrix *ctm, fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	if (dev->passthrough)
		fz_stroke_text(ctx, dev->passthrough, text, stroke, ctm, colorspace, color, alpha, cs_params);
}
...
struct shadearg
{
	fz_test_device *dev;
	fz_shade *shade;
	const fz_color_params *cs_params;
};
...
prepare_vertex(fz_context *ctx, void *arg_, fz_vertex *v, const float *color)
{
	...
	if (!shade->use_function)
		fz_test_color(ctx, dev, shade->colorspace, color, arg->cs_params);
}

static void
fz_test_fill_shade(fz_context *ctx, fz_device *dev_, fz_shade *shade, const fz_matrix *ctm, float alpha, const fz_color_params *cs_params)
{
	fz_test_device *dev = (fz_test_device*)dev_;
	...
	if (dev->resolved == 0)
	{
		...
		else
		{
			if (shade->use_function)
			{
				int i;
				for (i = 0; i < 256; i++)
					fz_test_color(ctx, dev, shade->colorspace, shade->function[i], cs_params);
			}
			else
			{
				struct shadearg arg;
				arg.dev = dev;
				arg.shade = shade;
				arg.cs_params = cs_params;
				fz_process_shade(ctx, shade, ctm, prepare_vertex, NULL, &arg);
			}
		}
	}
	if (dev->passthrough)
		fz_fill_shade(ctx, dev->passthrough, shade, ctm, alpha, cs_params);
}

static void
fz_test_fill_image(fz_context *ctx, fz_device *dev_, fz_image *image, const fz_matrix *ctm, float alpha, const fz_color_params *cs_params)
{
	fz_test_device *dev = (fz_test_device*)dev_;
	...
	if (dev->passthrough)
		fz_fill_image(ctx, dev->passthrough, image, ctm, alpha, cs_params);
}
	...
	fz_colorspace *colorspace, const float *color, float alpha, const fz_color_params *cs_params)
{
	...
	if (dev->passthrough)
		fz_fill_image_mask(ctx, dev->passthrough, image, ctm, colorspace, color, alpha, cs_params);
}
...
static void
fz_test_begin_mask(fz_context *ctx, fz_device *dev_, const fz_rect *rect, int luminosity, fz_colorspace *cs, const float *bc, const fz_color_params *cs_params)
{
	fz_test_device *dev = (fz_test_device*)dev_;

	fz_begin_mask(ctx, dev->passthrough, rect, luminosity, cs, bc, cs_params);
}
