...
typedef int Rune;
typedef struct fz_regex Reprog;
typedef struct fz_regmatch Resub;
...
{
	return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || fz_isalpha(c);
}
...
static int nextrune(struct cstate *g)
{
	...
	g->source += fz_chartorune(&g->yychar, g->source);
	if (g->yychar == '\\') {
		if (!*g->source)
...
Reprog *fz_regcomp(fz_context *ctx, const char *pattern, int cflags)
{
	struct cstate g;
	Renode *node;
	Reinst *split, *jump;
	...
	if (setjmp(g.kaboom)) {
		fz_free(ctx, g.pstart);
		if (g.prog) {
			fz_free(ctx, g.prog->cclass);
			fz_free(ctx, g.prog->start);
			fz_free(ctx, g.prog);
		}
		fz_throw(ctx, FZ_ERROR_ARGUMENT, "regcomp failure: %s", g.error);
	}

	g.prog = fz_malloc(ctx, sizeof (Reprog));
	if (!g.prog)
	...
	if (n > 0) {
		g.pstart = g.pend = fz_malloc(ctx, sizeof (Renode) * n);
		if (!g.pstart)
	...
	g.prog->nsub = g.nsub;
	g.prog->start = g.prog->end = fz_malloc(ctx, n * sizeof (Reinst));
	if (!g.prog->start)
		die(&g, "cannot allocate regular expression instruction list");

	if (g.ncclass > 0) {
		g.prog->cclass = fz_malloc(ctx, g.ncclass * sizeof (Reclass));
		if (!g.prog->cclass)
...
void fz_regfree(fz_context *ctx, Reprog *prog)
{
	if (prog) {
		if (prog->cclass)
			fz_free(ctx, prog->cclass);
		fz_free(ctx, prog->start);
		fz_free(ctx, prog);
	}
}
...
static int strncmpcanon(const char *a, const char *b, int n)
{
	...
	while (n--) {
		if (!*a) return -1;
		if (!*b) return 1;
		a += fz_chartorune(&ra, a);
		b += fz_chartorune(&rb, b);
		c = canon(ra) - canon(rb);
		if (c)
			return c;
	}
...
int fz_regexec(fz_context *ctx, Reprog *prog, const char *sp, Resub *sub, int eflags)
{
	Resub scratch;
	int runaway = 0;
	int i;
